...
import { haptic, openExternal, openTelegramUrl, telegramUser } from "./telegram";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function pull() {
      try {
        ...
        setMustSubscribe(value.channelRequired && !value.subscribed);
  ...
  function openInvite() {
    setError("");
    setAskBack(step === "invite" ? askBack : step);
    haptic("tap");
    setStep("invite");
  }
  ...
  function shareInvite() {
    if (!shareUrl) {
      setError(tx("inviteNeedTg"));
      haptic("error");
      return;
    }
    haptic("tap");
    openTelegramUrl(
      `https://t.me/share/url?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(tx("inviteShareText"))}`,
    );
  }
  ...
  async function checkRegistration() {
    ...
    try {
      ...
      setMustSubscribe(value.channelRequired && !value.subscribed);
      setChannelUrl(value.channelUrl);
      if (value.channelRequired && !value.subscribed) {
        setError(tx("subNo"));
        return;
      }
    ...
    <main className="app">
      ...
      {mustSubscribe ? (
        <section className="card">
          <p className="kicker">{tx("subKicker")}</p>
          <h2>{tx("subTitle")}</h2>
          <p className="copy">{tx("subCopy")}</p>
          ...
          <div className="actions">
            ...
            <button
              ...
              onClick={() => void checkSubscribe()}
            >
              {checkingSub ? tx("subWait") : tx("checkSub")}
            </button>
          </div>
        </section>
      ) : null}

      {!mustSubscribe && step === "gift" && (
        <section className="card">
          ...
          <div className="actions">
            ...
            <button className="cta secondary" type="button" onClick={openInvite}>
              {tx("inviteFriend")}
            </button>
          </div>
        </section>
      ...
      {!mustSubscribe && step === "invite" && (
        <section className="card">
          <p className="kicker">{credits === 0 ? tx("inviteKicker") : tx("inviteFriend")}</p>
          <h2>{tx("inviteTitle")}</h2>
          <p className="copy">
            {credits === 0
              ? tx("inviteCopy", { code: PROMO_CODE, bonus: BONUS_LABEL })
              : tx("inviteCopyAlways", { code: PROMO_CODE, bonus: BONUS_LABEL })}
          </p>
          {shareUrl ? <code className="invite-link">{shareUrl}</code> : <p className="error">{tx("inviteNeedTg")}</p>}
          {error ? <p className="error">{error}</p> : null}
          <div className="actions">
            {shareUrl ? (
              <button className="cta" type="button" onClick={shareInvite}>
                {tx("shareInvite")}
              </button>
            ) : null}
            {shareUrl ? (
      ...
      <div className="saldo-bar">
        ...
        <div className="saldo-actions">
          {!mustSubscribe && step !== "invite" ? (
            <button className="saldo-invite" type="button" onClick={openInvite}>
              {tx("inviteFriend")}
            </button>
          ) : null}
          {!mustSubscribe && step !== "ask" ? (
