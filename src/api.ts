...
export function withTracking(url: string, telegramId: number | null, referrerId = "") {
  if (!url) return url;
  try {
    const parsed = new URL(url);
    if (telegramId) parsed.searchParams.set("sub1", String(telegramId));
    if (referrerId && referrerId !== String(telegramId)) {
      parsed.searchParams.set("sub2", referrerId);
    }
    return parsed.toString();
  } catch {
    const params = [
      telegramId ? `sub1=${telegramId}` : "",
      referrerId && referrerId !== String(telegramId) ? `sub2=${referrerId}` : "",
    ]
      .filter(Boolean)
      .join("&");
...
export function pickOfferUrl(
  ...
  return withTracking(fromWebhook || baseOfferUrl(), telegramId, referrerId);
}
