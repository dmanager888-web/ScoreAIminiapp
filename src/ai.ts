...
export async function loadSaldo() {
  ...
  return {
    ...
    channelRequired: Boolean(data.channelRequired),
    channelUrl: data.channelUrl || "",
  };
}

export async function requestPick(text: string, image: ImagePayload | undefined, language: string) {
  const user = telegramUser();
  ...
  if (response.status === 403 || data.error === "not_subscribed") {
    return {
      ok: false as const,
